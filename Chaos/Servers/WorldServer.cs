    ...
    public ValueTask OnGroupRequest(IWorldClient client, in ClientPacket clientPacket)
    {
        ...
        ValueTask InnerOnGroupRequest(IWorldClient localClient, GroupRequestArgs localArgs)
        {
            ...
            switch (groupRequestType)
            {
                ...
                {
                    var existingGroup = localClient.Aisling.Group;

                    if (existingGroup != null)
                        existingGroup.Invite(localClient.Aisling, target);
                    else if (target.Group != null)
                    {
                        localClient.SendServerMessage(ServerMessageType.ActiveMessage, $"{target.Name} is already in a group");

                        return default;
                    } else if (!localClient.Aisling.Options.Group)
                        localClient.SendServerMessage(ServerMessageType.OrangeBar1, "You have elected not to join groups");
                    else if (!target.Options.Group)
                        localClient.SendServerMessage(ServerMessageType.OrangeBar1, $"{target.Name} refuses to join your group");
                    else
                        target.Client.SendGroupRequest(GroupRequestType.FormalInvite, localClient.Aisling.Name);
                ...
                case GroupRequestType.AcceptInvite:
                {
                    ...
                    if (existingGroup != null)
                        localClient.SendServerMessage(ServerMessageType.ActiveMessage, "You are already in a group");
                    else if (target.Group != null)
                        target.Group.AcceptInvite(target, localClient.Aisling);
                    else
                    {
                        var group = Group.Create(target, localClient.Aisling);
                        target.Group = group;
                        localClient.Aisling.Group = group;
                    }
