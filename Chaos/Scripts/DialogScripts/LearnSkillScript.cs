...
public class LearnSkillScript : ConfigurableDialogScriptBase
{
    ...
    public bool ValidateAndTakeRequirements(Aisling source, Dialog dialog)
    {
        ...
        if (template.Class.HasValue && !template.Class.Value.ContainsClass(source.UserStatSheet.BaseClass))
        {
            ...
            Logger.LogWarning("{Player} tried to learn skill {Skill} but is not the correct class", source, template.Name);
        ...
        {
            dialog.Reply(source, "Heh, nice try kid...");
            Logger.LogWarning("{Player} tried to learn skill {Skill} but is not the correct adv class", source, template.Name);
