...
internal static class Game
{
    ...
    internal static void Intialize(Server server)
    {
        ...
        Server = server;
        World = new World(Server, out var Init);
    ...
    internal static void RequestConnectionInfo(Client.Client client)
    {
        client.Enqueue(ServerPackets.ConnectionInfo(Server.TableCheckSum, client.Crypto.Seed, client.Crypto.Key));
    }

    internal static void CreateChar1(Client.Client client, string name, string password)
    {
    ...
    internal static void Login(Client.Client client, string name, string password)
    {
        ...
        else if (Server.TryGetUser(name, out var user))
        {
    ...
    internal static void CreateChar2(Client.Client client, byte hairStyle, Gender gender, byte hairColor)
    {
    ...
    internal static void RequestMapData(Client.Client client)
    {
        client.Enqueue(ServerPackets.MapData(client.User.Map));
    }

    internal static void ClientWalk(Client.Client client, Direction direction, int stepCount)
    {
        if (stepCount == client.StepCount)
            client.User.Map.ClientWalk(client, direction);
    }

    internal static void Pickup(Client.Client client, byte slot, Point groundPoint)
    {
    ...
    internal static void Drop(Client.Client client, byte slot, Point groundPoint, uint count)
    {
        var map = client.User.Map;
        ...
        //retreive the item
        if(client.User.Inventory.TryGetObject(slot, out var item))
        {
            ...
            var groundItem = item.ToGroundItem((client.User.Location.MapID, groundPoint), count);
    ...
    internal static void ExitClient(Client.Client client, bool requestExit)
    {
    ...
    internal static void Ignore(Client.Client client, IgnoreType type, string targetName)
    {
        switch(type)
        {
    ...
    internal static void PublicChat(Client.Client client, PublicMessageType type, string message)
    {
                ...
                if ((m = Regex.Match(message, @"^/([a-zA-Z]+)(?: ([a-zA-Z0-9]+))?")).Success)
                {
                    ...
                    switch (m.Groups[1].Value.ToLower())
                    {
                        case "commands":
                            ...
                            foreach (var cmd in CONSTANTS.COMMAND_LIST)
                        ...
                        case "face":
                            ...
                            break;
                        case "gender":
                            var str = Utilities.FirstUpper(m.Groups[2].Value);
    ...
    internal static void UseSpell(Client.Client client, byte slot, int targetId, Point targetPoint, string prompt)
    {
        var spell = client.User.SpellBook[slot];
    ...
    internal static void JoinClient(Client.Client client, byte seed, byte[] key, string name, uint id)
    {
        ...
        if (client.ServerType == ServerType.World)
        {
            ...
            foreach (var item in client.User.Inventory)
    ...
    internal static void Turn(Client.Client client, Direction direction)
    {
        //set the user's direction, and display the turn to everyone in range to see
        client.User.Direction = direction;
        foreach (var user in client.User.Map.ObjectsVisibleFrom(client.User.Point).OfType<User>())
            user.Client.Enqueue(ServerPackets.CreatureTurn(client.User.ID, direction));
    }

    internal static void SpaceBar(Client.Client client)
    {
    ...
    internal static void RequestWorldList(Client.Client client)
    {
        client.Enqueue(ServerPackets.WorldList(Server.WorldClients.Select(cli => cli.User).ToList(), client.User.Attributes.Level));
    }

    internal static void Whisper(Client.Client client, string targetName, string message)
    {
        //if the user isnt online, tell them
        if (!Server.TryGetUser(targetName, out var targetUser))
    ...
    internal static void ToggleUserOption(Client.Client client, UserOption option)
    {
    ...
    internal static void UseItem(Client.Client client, byte slot)
    {
        var item = client.User.Inventory[slot];

        if (item != null && item.CanUse)
            item.Activate(client, Server, new ActivationContext(item));
    }

    internal static void AnimateCreature(Client.Client client, BodyAnimation animNum)
    {
        foreach (var user in client.User.Map.ObjectsVisibleFrom(client.User.Point, true).OfType<User>())
            user.Client.Enqueue(ServerPackets.AnimateCreature(client.User.ID, animNum, 100));
    }

    internal static void DropGold(Client.Client client, uint amount, Point groundPoint)
    {
        var map = client.User.Map;
    ...
    internal static void ChangePassword(Client.Client client, string name, string currentPw, string newPw)
    {
    ...
    internal static void DropItemOnCreature(Client.Client client, byte slot, int targetId, byte count)
    {
        lock (ExchangeLock)
        {
            if (client.User.Map.TryGetObject(obj => obj.ID == targetId, out Creature creature) && client.User.Inventory.TryGetObject(slot, out var item) && item != null && creature.Point.Distance(client.User.Point) <= CONSTANTS.DROP_RANGE)
            {
    ...
    internal static void DropGoldOnCreature(Client.Client client, uint amount, int targetId)
    {
        lock (ExchangeLock)
        {
    ...
    internal static void RequestProfile(Client.Client client)
    {
        client.Enqueue(ServerPackets.ProfileSelf(client.User));
    }

    internal static readonly object GroupLock = new object();
    internal static void RequestGroup(Client.Client client, GroupRequestType type, string targetName, GroupBox box)
    {
        lock (GroupLock)
        {
            User targetUser;
                ...
                case GroupRequestType.TryInvite:
                    ...
                    else if (client.User.IsGrouped)                                                                                                                           //else if this we are already in a group
                    {
                        if (client.User == targetUser)                                                                                                                          //and we're trying to group ourself
                        {
                            ...
                            targetUser.Client.Enqueue(ServerPackets.GroupRequest(GroupRequestType.FormalInvite, client.User.Name));
                ...
                case GroupRequestType.AcceptInvite:
                    ...
                    else
                    {
                        ...
                        { //force group, hmmm
                            ...
                            {
                                foreach (var user in targetUser.Group)
    ...
    internal static void ToggleGroup(Client.Client client)
    {
    ...
    internal static void SwapSlot(Client.Client client, PanelType panelType, byte origSlot, byte endSlot)
    {
    ...
    internal static void RequestRefresh(Client.Client client)
    {
        client.Refresh();
    }

    internal static void RequestPursuit(Client.Client client, GameObjectType objType, int objId, PursuitId pursuitId, byte[] args)
    {
    ...
    internal static void ReplyDialog(Client.Client client, GameObjectType objType, int objId, PursuitId pursuitId, ushort dialogId, DialogArgsType argsType, byte option, string userInput)
    {
        var dialog = client.CurrentDialog;
        var dialogPursuit = dialog.PursuitId;
        option--;
        ...
        var opt = Enum.IsDefined(CONSTANTS.DIALOGOPTION_TYPE, Convert.ToSByte(dialogId - dialog.Id)) ? (DialogOption)(dialogId - dialog.Id) 
            : DialogOption.Close;
            ...
            case DialogOption.Next:
                switch(argsType)
                {
                    ...
                    case DialogArgsType.MenuResponse:
                        var menuItem = client.CurrentDialog.Menu[option];
                        if (menuItem.PursuitId != PursuitId.None)
                        {
    ...
    internal static void Boards(Client.Client client)
    {
        //work in progress
        client.Enqueue(ServerPackets.BulletinBoard());
    }

    internal static void UseSkill(Client.Client client, byte slot)
    {
        var skill = client.User.SkillBook[slot];

        if (skill != null && skill.CanUse && !client.User.HasFlag(Status.Dead))
            skill.Activate(client, Server, new ActivationContext(skill));
    }

    internal static void ClickWorldMap(Client.Client client, ushort nodeCheckSum, Location location)
    {
    ...
    internal static void ClickObject(Client.Client client, int objectId)
    {
    ...
    internal static void ClickObject(Client.Client client, Point clickPoint)
    {
    ...
    internal static void RemoveEquipment(Client.Client client, EquipmentSlot slot)
    {
        ...
        if (client.User.Equipment.TryGetRemove((byte)slot, out var item))
        {
    ...
    internal static void KeepAlive(Client.Client client, byte a, byte b)
    {
        client.Enqueue(ServerPackets.KeepAlive(a, b));
    }

    internal static void ChangeStat(Client.Client client, Stat stat)
    {
        switch(stat)
        {
            case Stat.STR:
    ...
    internal static void Exchange(Client.Client client, ExchangeRequestType requestType, int targetId = 0, uint goldAmount = 0, byte itemSlot = 0, byte itemCount = 0)
    {
        switch (requestType)
        {
    ...
    internal static void RequestLoginNotification(bool check, Client.Client client)
    {
        client.Enqueue(ServerPackets.LoginNotification(check, Server.LoginMessageCheckSum, Server.LoginMessage));
    }

    internal static void BeginChant(Client.Client client)
    {
        if (!client.User.HasFlag(Status.Dead))
    ...
    internal static void DisplayChant(Client.Client client, string chant)
    {
        foreach (var user in client.User.Map.ObjectsVisibleFrom(client.User.Point, true).OfType<User>())
            user.Client.SendPublicMessage(PublicMessageType.Chant, client.User.ID, chant);
    }

    internal static void Personal(Client.Client client, byte[] portraitData, string profileMsg)
    {
        if (portraitData.Length > 0)
            using (var imageData = new MemoryStream(portraitData))
                if (imageData.Length != 48 * 56)
                    portraitData = new byte[] { };

        client.User.Personal = new Personal(portraitData, profileMsg);
    }

    internal static void RequestServerTable(Client.Client client, bool request)
    {
        if (request)
    ...
    internal static void RequestHomepage(Client.Client client)
    {
        client.Enqueue(ServerPackets.LobbyControls(3, @"http://www.darkages.com"));
    }

    internal static void SynchronizeTicks(Client.Client client, TimeSpan serverTicks, TimeSpan clientTicks)
    {
        client.Enqueue(ServerPackets.SynchronizeTicks());
    }

    internal static void ChangeSocialStatus(Client.Client client, SocialStatus status)
    {
        client.User.SocialStatus = status;
    }

    internal static void RequestMetaFile(Client.Client client, bool all)
    {
