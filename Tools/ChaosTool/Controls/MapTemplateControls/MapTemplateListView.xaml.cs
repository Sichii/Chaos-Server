...
public sealed class MapTemplateListView : ViewModelListView
{
    public MapTemplateListView() => InitializeComponent();
    ...
    {
        var mapped = JsonContext.MapTemplates
                                .Objects
                                .Select(obj => obj.MapTo<MapTemplateViewModel>())
    ...
    protected override async void AddButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        var viewModel = new MapTemplateViewModel
        {
            OriginalPath = path,
            Path = path
        };
