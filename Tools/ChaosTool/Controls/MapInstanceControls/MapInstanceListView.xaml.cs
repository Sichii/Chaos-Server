...
public sealed class MapInstanceListView : ViewModelListView
{
    public MapInstanceListView() => InitializeComponent();
    ...
    {
        var mapped = JsonContext.MapInstances
                                .Objects
                                .Select(obj => obj.MapTo<MapInstanceViewModel>())
    ...
    protected override async void AddButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        var viewModel = new MapInstanceViewModel
        {
            OriginalPath = path,
            Path = path
        };
