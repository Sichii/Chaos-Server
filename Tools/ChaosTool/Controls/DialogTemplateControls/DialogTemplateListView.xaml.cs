...
public sealed class DialogTemplateListView : ViewModelListView
{
    public DialogTemplateListView() => InitializeComponent();
    ...
    {
        var mapped = JsonContext.DialogTemplates
                                .Objects
                                .Select(obj => obj.MapTo<DialogTemplateViewModel>())
    ...
    protected override async void AddButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        var viewModel = new DialogTemplateViewModel
        {
            OriginalPath = path,
            Path = path
        };
