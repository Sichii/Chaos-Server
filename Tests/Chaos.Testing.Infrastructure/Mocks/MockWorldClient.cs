...
public sealed class MockWorldClient : WorldClientBase
{
    public List<IPacketSerializable> SentArgs { get; } = [];

    private MockWorldClient(
        Socket socket,
        ICrypto crypto,
        IPacketSerializer packetSerializer,
        ILogger<MockWorldClient> logger)
        : base(
            socket,
            crypto,
            packetSerializer,
    ...
    public static MockWorldClient Create(Socket? socket = null, ICrypto? crypto = null, IPacketSerializer? packetSerializer = null)
    {
        if (socket is null)
        {
